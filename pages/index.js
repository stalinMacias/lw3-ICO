...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (!walletConnected) {
      ...
      // The withdraw() function from the CryptoDevTokens contract must be called only by the contract's Owner, there is no a valid reason to leave the below function invocation to be executed each time there is a change in the wallet
	    // A good idea might be to add a couple validations and enable the calling of the withdrawCoins() method only when the connected account it the contract's owner, other than that, any other account should even try to call that method since all those calls will be rejected because the withdraw() function implemented the onlyOwner modifier!!!
      //withdrawCoins();
    }
  }, [walletConnected]);
